...
router.put(
  "/user/email/change",
  userAuthentication.authMiddleware,
  async (request: Request, resposne: Response) => {
    try {
      const { email, password } = request.body;
      const token = request.headers["authorization"]?.split(" ")[1];
      const result = await userController.changeEmail(
        token as string,
        email as string,
        password as string
      );
...
router.put(
  ...
  async (request: Request, response: Response) => {
    try {
      ...
      const result = await userController.changeImg(
        img as string,
        token as string
      );
      response.status(200).json(result);
...
router.put(
  ...
  async (request: Request, response: Response) => {
    try {
      ...
      const result = await userController.changeName(
        name as string,
        token as string
      );
      response.status(200).json(result);
