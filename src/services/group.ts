...
class GroupService {
  ...
  public createGroup = async (
    ...
    try {
      ...
      await this.groupUserService.addUser(userId, "admin", groupId, "admin");
      return {
  ...
  public deleteGroup = async (id: string, userId: string) => {
    const user = (await this.groupUserService.getUser(id, userId)) as {
      role: string;
    };
    const role = user?.role as string;
    if (role === "admin") {
      try {
        ...
        await this.groupUserService.deleteGroupUsers(id);
        return {
